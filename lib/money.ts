...
export function formatMoney(money?: Money) {
  ...
  return new Intl.NumberFormat(
    localeForCurrency[money.currencyCode] ?? "en-US",
    {
      style: "currency",
...
export function selectPublicPrice(
  ...
  return (
    prices?.find(
      (price) =>
        price.country === country && !price.customerGroup && !price.channel,
    ) ??
    prices?.find(
      (price) => !price.country && !price.customerGroup && !price.channel,
    ) ??
